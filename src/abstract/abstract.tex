...
\begin{document}
    \begin{abstract}
        ...
        This article measures the impact of the cold-start problem on GPU workloads (both in hardware and in simulator), and proposes some mitigations. % TODO: perhaps only one?
    \end{abstract}
    ...
    \begin{itemize}
        ...
        We will also compare these methods against one another, to see which one is the most effective. % TODO: rewrite if only one
    \end{itemize}
    ...
    \section{Mitigations}\label{sec:mitig} % TODO: singular if only one
