...
Before continuing with the cold-start problem, we'll shortly discuss the current state of GPU simulation.
...
Some parameters can't be determined by the benchmarks themselves, like warp scheduling policy, memory scheduling, and some L2 cache parameters (interleaving and hash function).
...
During simulation, it's quite often the case to have thousands of in-flight memory requests from hundreds of active threads.
...
To really make a difference in simulation time, you'd need to curtail the number of threads, rather than the number of basic blocks per thread.
...
While PKA is a very powerful technique, leading to drastically lower simulation times while still maintaining a high accuracy for most application, it isn't perfect yet.
